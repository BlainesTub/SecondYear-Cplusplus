...
std::list<int> add_lists(std::list<int> list1, std::list<int> list2) {
	...
	for(auto indx {list1.rbegin()}; indx !=list1.rend(); ++indx){
		lv1.push_back(*indx);
	}
	for(auto indx {list2.rbegin()}; indx !=list2.rend(); ++indx){
		lv2.push_back(*indx);
	}
	carry = 0;
	for(unsigned long i = 0; i < lv1.size(); ++i){
		sum = lv1.at(i)+lv2.at(i) + carry;
		
		//std::cout<<lv1.at(i)<< "+" << lv2.at(i)<< "+" <<  carry<< "="<< sum<< " ";
		
		if(sum >9){
			remain = sum - 10;

			sumlist.push_front(remain);
			carry = 1;
		}
		else{
...
int main() {
	std::list<int> test_numbers {211, 116, 265, 475, 999};
	int test_id {1};
