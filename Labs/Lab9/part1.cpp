...
template<typename T>
typename T::value_type sum_vector(T const& V){  //T = std::vector<int>. We have a vector of pointers called V
...
*/
template<typename T>                              //create mystery typename T
void print_vector(std::vector<T> const& V) {      //It is given a constant pointer to a contaner V
    for (auto x : V) {                            //Iterate through v
        std::cout << x << ", ";
    }
