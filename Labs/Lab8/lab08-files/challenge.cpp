...
public:
    ...
    */
    void remove_node(std::string str) {
        ...
        auto current = head;
        auto last = current;
        if (head->get() == str) head = head->get_next();
        ...
        while(current != tail){
            ...
            if (current->get() == str){
                if(current == tail){
                    ...
                    current = last; //this is the end. do not go farther
            ...
            last = current;   //if the current was the old tail, the while loop wil lbreak
        }
    }
